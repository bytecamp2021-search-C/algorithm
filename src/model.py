...
def diversity_recall_single_thread(items,users,tags,group_indices,group_algos,tag_sets=None,diversity_coef=0.1,num_reserve=100):
	results = []
...
def diversity_recall(items,users,tags,group_indices,group_algos,tag_sets=None,num_recall=10):
	...
	for k in range(num_thread):
		threads.append(MyThread(k,diversity_recall_in_each_thread,\
				[k*datasize,min((k+1)*datasize,len(users))],items,users,tags,group_indices,group_algos,num_recall,tag_sets))
		threads[k].start()
